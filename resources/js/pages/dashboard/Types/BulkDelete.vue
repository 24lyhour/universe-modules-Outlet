...
const form = useForm({
    ids: props.typeItems.map((t) => t.id),
});
...
const handleSubmit = () => {
    form.delete('/dashboard/outlet-types/bulk-delete', {
        preserveScroll: true,
        onSuccess: () => {
            toast.success(`${props.typeItems.length} outlet type(s) deleted successfully.`);
            setTimeout(() => {
                close();
                redirect();
            }, 100);
...
<template>
    <ModalForm
        v-model:open="isOpen"
        :title="`Delete ${typeItems.length} Outlet Type${typeItems.length > 1 ? 's' : ''}`"
        description="This action will move the selected outlet types to trash"
        mode="delete"
        size="md"
        :submit-text="`Delete ${typeItems.length} Type${typeItems.length > 1 ? 's' : ''}`"
        ...
        <div class="space-y-6">
            ...
            <div class="space-y-2">
                <p class="text-sm font-medium text-muted-foreground">
                    The following outlet types will be deleted:
                </p>
                <div class="max-h-48 space-y-2 overflow-y-auto rounded-lg border p-3">
                    ...
                    >
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="truncate text-sm font-medium">{{ type.name }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-start gap-3 rounded-lg border border-destructive/50 bg-destructive/10 p-4">
                ...
                <div class="space-y-1">
                    <p class="text-sm font-medium text-destructive">
                        You are about to delete {{ typeItems.length }} outlet type{{ typeItems.length > 1 ? 's' : '' }}
                    </p>
            ...
            <div class="flex items-start space-x-3 rounded-lg border p-4">
                ...
                <div class="space-y-1">
                    ...
                    <p class="text-sm text-muted-foreground">
                        I understand that {{ typeItems.length }} outlet type{{ typeItems.length > 1 ? 's' : '' }} will be deleted.
                    </p>
                </div>
            </div>

            <p v-if="form.errors.ids" class="text-sm text-destructive">
                {{ form.errors.ids }}
            </p>
        </div>
    </ModalForm>
