...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Trash', href: '/dashboard/outlets/trash' },
];

const trashConfig: TrashConfigLocal = {
    entityLabel: 'Outlet',
    entityLabelPlural: 'Outlets',
    restoreRoute: (uuid: string) => `/dashboard/outlets/${uuid}/restore`,
    forceDeleteRoute: (uuid: string) => `/dashboard/outlets/${uuid}/force-delete`,
    listRoute: '/dashboard/outlets/trash',
};
...
const handlePageChange = (page: number) => {
    router.get('/dashboard/outlets/trash', {
        page,
...
const handleBulkRestore = () => {
    router.put('/dashboard/outlets/trash/bulk-restore', {
...
const handleBulkForceDelete = () => {
    if (confirm(`Are you sure you want to permanently delete ${selectedUuids.value.length} item(s)? This action cannot be undone.`)) {
        router.delete('/dashboard/outlets/trash/bulk-force-delete', {
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head title="Outlets Trash" />

        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            <!-- Header -->
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold">Outlets Trash</h1>
                    <p class="text-sm text-muted-foreground">
                        Manage deleted outlets - restore or permanently delete
                    </p>
                </div>
                <ButtonGroup>
            ...
            <TrashTable
                ...
                :selectable="true"
                select-key="uuid"
                empty-message="No deleted outlets found."
                empty-trash-route="/dashboard/outlets/trash/empty"
